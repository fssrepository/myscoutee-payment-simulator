...
<body><main><img class="provider-logo" src="/simulator-ui/barion.svg" alt="Barion">{{if .Pending}}<h1>Was this you?</h1><p class="warning">Simulated bank confirmation. MyScoutee does not control whether this challenge is required.</p>
...
</div>{{else}}<div data-confirmation-complete="true"><h1>Bank authentication result</h1><p>Status: <span class="status">{{.Payment.Status}}</span></p></div>{{end}}
